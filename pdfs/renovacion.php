...
LEFT JOIN historico_renovaciones_$sucursal ON historico_renovaciones_$sucursal.lote = lotes_$sucursal.id_lote
WHERE id_renovaciones = $idrenovacion 
";
...
<div style="text-align: justify; font-style: italic; margin:15px 0 15px 0;font-size:6pt;">Declaro que las piezas aquí detalladas, son de mi propiedad lícita y se hallan libres de toda carga. Doy conformidad con el precio fijado por las piezas que vendo, y que se encuentran arriba descritas.</div>
...
$querys = "SELECT * FROM articulos_$sucursal WHERE id_lote_articulos =". $rsItem['id_lote'] ;
mysql_query ("SET NAMES 'utf8'");
...
$quer = "SELECT * FROM articulos_$sucursal WHERE id_lote_articulos =". $rsItem['id_lote'] ;
mysql_query ("SET NAMES 'utf8'");
...
$querys = "SELECT * FROM historico_renovaciones_$sucursal WHERE lote =". $rsItem['id_lote'] ." and id_renovaciones <= $idrenovacion" ;
mysql_query ("SET NAMES 'utf8'");
$Itemas = mysql_query($querys, $conexion);
$i = 1;
while($row = mysql_fetch_assoc($Itemas)){ 
...
if ($fecharenovacion != '0000-00-00') {
  $fecharenovacion=date('d-m-Y',strtotime($fecharenovacion));
}else{
  $fecharenovacion = '-----';
}

$fecha_vencido= $row['fecha_vencido'];
if ($fecha_vencido != '0000-00-00') {
  $fecha_vencido=date('d-m-Y',strtotime($fecha_vencido));
}else{
  $fecha_vencido = '-----';
}
...
<td align="center">'.$row['importe_renovacion'].' €</td>
<td align="center">'.$fecha_vencido.'</td>
</tr>';
...
<div style="text-align: justify; font-style: italic; margin:0 0 15px 0; font-size:7pt;line-height: 150%;">Declaro que las piezas aquí detalladas, son de mi propiedad lícita y se hallan libres de toda carga. Doy conformidad con el precio fijado por las piezas que vendo, y que se encuentran arriba descritas.
Las partes se reconocen mutua capacidad para celebrar y obligarse a tal efecto. Por el presente documento el VENDEDOR declara que: 1. Dicho bien pertenece al VENDEDOR que no procede de acto
ilícito y que no está afectado por ningún tipo de traba, garantía o embargo sobre el mismo. 2. Que el referido bien se encuentra en perfecto estado de uso y conservación acorde con su naturaleza y
antigüedad. 3. Que el VENDEDOR asume el compromiso de reembolsar al COMPRADOR en el plazo estipulado en el contrato el importe de recompra acordado así como los daños y perjuicios,
incluido el lucro cesante, que pudiera derivarse contra el VENDEDOR en el caso que lo manifestado anteriormente fuere incorrecto o inexacto produciéndose cualquier tipo de reclamación de terceros
sobre el bien trasmitido en el presente contrato o este fuere inservible para el uso ordinario por causas imputables al VENDEDOR.Ambas partes acuerdan la celebración de un contrato de compra y venta
con pacto de opción a recompra, lo que llevan a efecto conforme a las siguientes condiciones: 1. El VENDEDOR vende al COMPRADOR el elemento descrito en precio y condiciones arriba señaladas
sirviendo el presente documento de total y eficaz carta de pago. 2. La presente compra y venta no está sujeta a ningún tipo especial de condición por lo que desde ahora es plenamente válida y eficaz,
entendiendo el VENDEDOR que ya no es propietario de los bienes a ningún efecto. 3. El COMPRADOR concede al VENDEDOR una opción de compra sobre el o los artículos antes señalados la cual
deberÃ¡ ejercitarse antes de la fecha de vencimiento anteriormente concretada. Si la fecha de vencimiento coincidiera con un Domingo o festivo, el COMPRADOR deberá ejercitar la opción de recompra,
como maximo, el dia laborable inmediatamente anterior, al de la finalizacion del contrato. Transcurrido el plazo, quedara sin efecto la opción, cancelando automáticamente sin necesidad de notificación de
ninguna clase. 4. La presente opción de compra se otorga a petición y como facultad al VENDEDOR, quien acepta libremente con pleno conocimiento de sus consecuencias y sin que en su decisión
influya una situación económica tal que le obligue a usar esta opción como alternativa a los medios ordinarios de crédito, asumiendo que el comprador ofrece la opción basada en estas
manifestaciones. 5. El VENDEDOR optante podrá ceder su derecho de opción por cualquier título sin necesidad de aprobación por parte del COMPRADOR. No obstante, deberá poner, personalmente,
en conocimiento del COMPRADOR, el hecho de la cesión y las circunstancias personales del tercero adquiriente, dejando constancia escrita de su cesión y notificación al COMPRADOR. Ejecutada la
cesión, el cesionario, como ejercita el mismo derecho de cedente, no podrá exigir del COMPRADOR la garantía regulada en la Ley 23/2003, al quedar excluido el derecho de opción cedido de lo
establecido en dicha ley. 6. Salvo pacto expreso en contrario el precio de venta del bien ( ya anteriormente concretado) deberá de hacerse efectivo en el momento de ejercitar la opción . 7. El
COMPRADOR se obliga por su parte a no transmitir el objeto sujeto a la opción durante el plazo de vigencia de la misma y a mantenerlo en el mismo estado de uso y conservación en el que fue entregado.
Si por cualquier circunstancia no imputable al COMPRADOR, especialmente por perdida, deterioro o substracción, no fuese posible transmitirle los objetos sujetos a la operación de compra, el
VENDEDOR, tendrá derecho a ser reembolsado por las cantidades que hubiese podido satisfacer al COMPRADOR derecho a esta opción de compra o posibles prórrogas, junto con el equivalente al diez
por ciento del precio de compra satisfecho por el COMPRADOR al adquirir este objeto. 8. Ese producto pasará controles y el caso de ser intervenido policial o judicialmente, no podrá ser recuperado hasta
tener la autorización de los correspondientes entes. 9. El tiempo minimo de retirada lo marcan los Cuerpos y Fuerzas de Seguridad del Estado y oscilan entre los 15 y 30 dias en los casos habituales,
pudiendo ser este plazo superior en casos excepcionales. 10. En caso de robo en nuestras instalaciones nos veremos obligados a pagar el 10% del valor de recompra a los beneficiarios de los contratos de
recompra. Y en prueba de conformidad y aceptación firman el presente documento por duplicado, en el lugar y la fecha indicada:</div>
