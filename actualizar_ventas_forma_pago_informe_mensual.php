<?php
/**
 * CLI only (SSH): rellena en informe_mensual las columnas de ventas por forma
 * de pago sumando desde informe_semanal.
 *
 * Uso:
 *   php actualizar_ventas_forma_pago_informe_mensual.php
 *   php actualizar_ventas_forma_pago_informe_mensual.php --year=2025
 *   php actualizar_ventas_forma_pago_informe_mensual.php --year=2026
 *
 * Recomendación: ejecutar antes actualizar_ventas_forma_pago_informe_semanal.php
 */
...
echo ">> Inicio: actualizar ventas forma pago informe_mensual\n";
...
$sqlInformes = 'SELECT id_informe, sucursal_informe, numero_mes, year_informe
...
$sqlInformes .= ' ORDER BY year_informe ASC, numero_mes ASC, sucursal_informe ASC';
...
if (!$resultadoInformes) {
    fwrite(STDERR, "ERROR: sin resultado de informes mensuales.\n");
    mysqli_stmt_close($stmtInformes);
    mysqli_close($conexion);
    exit(1);
}
...
if (!$stmtSum) {
    fwrite(STDERR, 'ERROR preparando SUM semanal: ' . mysqli_error($conexion) . "\n");
    mysqli_stmt_close($stmtInformes);
    mysqli_close($conexion);
    exit(1);
}

$sqlUpdate = 'UPDATE informe_mensual
              SET ventas_contado = ?,
...
while ($informe = mysqli_fetch_assoc($resultadoInformes)) {
    ...
    mysqli_stmt_bind_param($stmtSum, 'isi', $numeroMes, $yearInforme, $sucursalInforme);
