...
$tabla_items_articulos_simp = $tablasSimp['articulos'];
$tabla_items_renovaciones_simp = $tablasSimp['renovaciones'];

if ($factura_mpdf_buffer && $id_factura > 0) {
    if (fiskalyEsFacturaFiskaly(
        (string) ($infoOrigenSimp['factura_regimen'] ?? 'false'),
        (int) ($infoOrigenSimp['id_rel_factura_fiskaly'] ?? 0)
...
$id_rel_factura_fiskaly = (int) ($infoOrigenSimp['id_rel_factura_fiskaly'] ?? 0);
$factura_regimen = (string) ($infoOrigenSimp['factura_regimen'] ?? 'false');
...
if ($tipo_factura === 'renovaciones') {
    ...
    FROM ' . $tabla_items_renovaciones_simp . ' frr
    ...
    FROM ' . $tabla_items_articulos_simp . ' fra
