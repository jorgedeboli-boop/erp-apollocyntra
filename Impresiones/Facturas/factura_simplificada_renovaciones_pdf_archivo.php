<?php
/**
 * PDF factura simplificada de renovaciones — adaptación de `factura_simplificada_renovaciones.php`
 * para FACTURA_MPDF_BUFFER_MODE, mysqli y mPDF (Composer).
 *
...
$origen_get = isset($_GET['origen']) ? strtolower(trim((string) $_GET['origen'])) : '';
if ($origen_get === 'unificada') {
    $origen_get = 'facturas';
}
$preferencia_origen = ($origen_get === 'facturas' || $origen_get === 'historico') ? $origen_get : '';
$infoOrigenSimp = facturaSimplificadaResolverOrigen($id_factura, $preferencia_origen);
if (!$infoOrigenSimp) {
    if ($factura_mpdf_buffer) {
        return null;
    }
...
$tablasSimp = facturaSimplificadaTablasPorOrigen($infoOrigenSimp['origen']);
$tabla_cabecera_simp = $tablasSimp['cabecera'];
$tabla_items_renovaciones_simp = $tablasSimp['renovaciones'];

if ($factura_mpdf_buffer && $id_factura > 0) {
    if (fiskalyFacturaVinculadaEnCache((int) ($infoOrigenSimp['id_rel_factura_fiskaly'] ?? 0))
        && fiskalyScriptImpresionFactura((string) ($infoOrigenSimp['factura_regimen'] ?? 'false'), true) === 'factura_Bizkaia_tbai.php') {
        $_GET['tipo'] = 'simplificada';
        $_GET['origen'] = $infoOrigenSimp['origen'] === 'facturas' ? 'facturas' : 'historico';
...
$id_factura = (int) $rsItemFR['id_factura'];
...
$sucursal_factura = (int) $rsItemFR['sucursal_factura'];
$logotipoPdf = $rsItemFR['logotipo_sucursal'];
$fecha_factura = $rsItemFR['fecha_factura'];
$fecha_factura_final = date('d-m-Y', strtotime((string) $fecha_factura));
$fecha_factura_parset = strtotime((string) $fecha_factura);
$anyo_factura = $fecha_factura_parset ? date('Y', $fecha_factura_parset) : date('Y');
...
$id_empresa = (int) $rsItemFR['id_empresa'];
...
$id_rel_factura_fiskaly = (int) ($infoOrigenSimp['id_rel_factura_fiskaly'] ?? ($rsItemFR['id_rel_factura_fiskaly'] ?? 0));
$factura_regimen = (string) ($infoOrigenSimp['factura_regimen'] ?? ($rsItemFR['factura_regimen'] ?? 'false'));
...
$query_ren = 'SELECT id_rel_renovacion, precio_venta_sin_iva, precio_rel_renovacion
    FROM ' . $tabla_items_renovaciones_simp . '
    WHERE id_rel_factura = ?
    ORDER BY id_rel_fac_art ASC
    LIMIT 1';
$stmtRen = mysqli_prepare($conexion, $query_ren);
$precio_venta_sin_iva = 0.0;
$precio_rel_renovacion = 0.0;
$iva_total = 0.0;
if ($stmtRen) {
    mysqli_stmt_bind_param($stmtRen, 'i', $id_factura);
    mysqli_stmt_execute($stmtRen);
    $rr = mysqli_stmt_fetch_assoc_compat($stmtRen);
    mysqli_stmt_close($stmtRen);
    if ($rr) {
        $precio_venta_sin_iva = (float) ($rr['precio_venta_sin_iva'] ?? 0);
        $precio_rel_renovacion = (float) ($rr['precio_rel_renovacion'] ?? 0);
...
$headerTabla = '
    <table width="100%" style="margin-top:20px;">
        <tr>
            <td width="50%">' .
                (!empty($logotipoPdf)
                    ? '<img src="../../photos/' . htmlspecialchars((string) $logotipoPdf, ENT_QUOTES, 'UTF-8') . '" width="300" height="auto" alt="">'
                    : '&nbsp;') . '
            </td>
            <td width="50%" style="text-align: right; vertical-align: bottom;">
               <p class="textoheader">' . htmlspecialchars((string) $nombre_empresa, ENT_QUOTES, 'UTF-8') . '</p>
               <p class="textoheader">CIF: ' . htmlspecialchars((string) $cif_empresa, ENT_QUOTES, 'UTF-8') . '</p>
...
$queryFRA = 'SELECT * FROM ' . $tabla_items_renovaciones_simp . ' WHERE id_rel_factura = ? ORDER BY id_rel_fac_art ASC';
...
if ($stmtFRA) {
    ...
    foreach ($fraRows as $row) {
        ...
        if ($precio_sin_iva_linea <= 0.0 && $precio_rel_linea > 0.0) {
            $precio_sin_iva_linea = round($precio_rel_linea / 1.21, 2);
        }
...
if (!empty($sello_image)) {
    $sello_img_html = '<div style="width: 330px; position: absolute; bottom: 74px; left: 88px; ">
        <img src="../../photos/' . htmlspecialchars((string) $sello_image, ENT_QUOTES, 'UTF-8') . '" width="188" alt="">
    </div>';
}
...
$mpdfone->OutputHttpDownload('factura_simplificada_renovaciones_' . $id_factura . '.pdf');
exit;
