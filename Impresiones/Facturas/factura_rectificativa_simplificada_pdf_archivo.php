<?php
/**
 * PDF factura rectificativa de facturas simplificadas (tabla facturas_rectificativas_simplificadas).
 *
...
if (!$factura_mpdf_buffer) {
    fiskalyRedirigirImpresionDesdeGet($id_factura, 'facturas_rectificativas_simplificadas', true);
}
...
if ($factura_mpdf_buffer && $id_factura > 0) {
    ...
    if ($conexionFiskaly) {
        $stmtFsk = mysqli_prepare(
            $conexionFiskaly,
            'SELECT factura_regimen, id_rel_factura_fiskaly FROM facturas_rectificativas_simplificadas WHERE id_factura = ? LIMIT 1'
        );
        if ($stmtFsk) {
            ...
            if ($rowFsk && fiskalyFacturaVinculadaEnCache((int) ($rowFsk['id_rel_factura_fiskaly'] ?? 0))
                ...
                $_GET['tipo'] = 'rectificativa_simplificada';
...
$queryFR = "SELECT
    ...
    dc.firma_cliente
FROM facturas_rectificativas_simplificadas AS FCTR
...
$prefijo_factura_original = $rsItemFR['prefijo_factura_original'];
$factura_original = isset($rsItemFR['factura_original']) ? trim((string) $rsItemFR['factura_original']) : '';
$factura_original = $prefijo_factura_original."-".$factura_original;
$motivo_rectificado = isset($rsItemFR['motivo_rectificado']) ? trim((string) $rsItemFR['motivo_rectificado']) : '';
...
$qr_img_html = fiskalyHtmlImagenQr($fiskalyQr['imagen_codigo_qr']);
$factura_regimen_qr = (string) ($rsItemFR['factura_regimen'] ?? 'false');

// VAR ORIGINALES DEL DOCUMENTO
$titulodocumento = 'Factura rectificativa';
...
$mpdfone->SetTitle('Factura rectificativa ' . $nombre_empresa);
...
// ====== BEGIN: HTML del original (copiado) ======
$html = '
<html>
<head>
<meta charset="UTF-8" />
	<style>
			...
			body { background: #ffffff; font-family: chelvetica; font-size: 9pt; width:900px; margin:0 auto; color:#555555; }
			p { margin: 0pt; }
			td { vertical-align: top; }
			.items td { }
			table thead td { background-color: #EEEEEE; text-align: center; }
			.items td.blanktotal { background-color: none; border: none; }
			.items td.totals { text-align: right; }
			.textoheader{ font-size:12px; height: 31.3pt; color:#555555; }
			.textofooter{ text-align:left; width: 350px; float:left; display: block; line-height: 24px; background-color: red; }
			.paginacionfooter{ background-color: #EEEEEE; width: 20px; text-align:right; float: right; display: block; line-height: 24px; }
			.titulodocumento{ color:#555555; font-size: 40px; text-align: left; display: block; width: 100%; float: left; border-bottom: 2px solid '.$appColorBrand.'; line-height: 44px; margin:0px; }
			.texto-a{ color: '.$appColorBrand.'; font-style: italic; font-size: 17px; font-weight: normal; text-align: right; line-height: 29px; margin-right:5px; }
			.texto-b{ color: #555555; font-style: normal; font-size: 17px; font-weight: normal; line-height: 29px; }
			.adestino{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 27px; font-weight: normal; text-align: right; line-height: 20px; display: block; width: 60px; padding: 0px 3px 5px 0px; height: 20px; margin: 0px; float: left; }
			.destinatariodata{ float:left; width:390px; height:auto; text-align:left; }
			.texto-c{ color: #555555; font-style: normal; font-size: 14px; font-weight: normal; line-height: 25px; }
			.nombrecliente{ color: #555555; font-style: normal; font-size: 16px; font-weight: bold; line-height: 20px; }
			.totaltopleft{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 27px; font-weight: bold; text-align: right; line-height: 45px; display: block; width: 174px; padding: 0px 13px 5px 0px; height: 45px; margin: 0px; float: left; }
			.totaltopright{ background: #EDEDEE; color: '.$appColorBrand.'; font-style: normal; letter-spacing:-1px; font-size: 22px; font-weight: bold; text-align: center; line-height: 45px; display: block; width: 143px; padding: 0px 3px 5px 0px; height: 45px; margin: 0px; float: right; }
			.itemnumbernav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 70px; padding: 0px 0px 0px 0px; height: 28px; margin: 0px; float: left; }
			.itemdescripcionnav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 248px; padding: 0px 0px 0px 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemunidadesnav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 80px; padding: 0px 0px 0px 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itempreciounitarionav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 90px; padding: 0px 0px 0px 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemivanav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 91px; padding: 0px 0px 0px 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemtotalnav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 13px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 146px; padding: 0px 0px 0px 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemsinvoice{ font-style: normal; font-size: 13px; font-weight: normal; text-align: left; display: block; width: 100%; padding: 0px 5px; height: 30px; margin: 0px 0px 5px 0px; }
			.pitemnumbernav{ font-style: normal; font-size: 13px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 60px; padding: 0px 0px 0px 0px; height: 23px; margin: 0px; float: left; }
			.pitemdescripcionnav{ font-style: normal; font-size: 12px; font-weight: normal; text-align: left; line-height: 20px; display: block; width: 242px; padding: 0px 0px 0px 0px; height: 23px; margin: 0px 0px 0px 14px; float: left; }
			.pitemunidadesnav{ font-style: normal; font-size: 13px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 70px; padding: 0px 0px 0px 0px; height: 23px; margin: 0px 0px 0px 14px; float: left; }
			.pitempreciounitarionav{ font-style: normal; font-size: 13px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 81px; padding: 0px 0px 0px 0px; height: 23px; margin: 0px 0px 0px 14px; float: left; }
			.pitemivanav{ font-style: normal; font-size: 13px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 100px; padding: 0px 0px 0px 0px; height: 23px; margin: 0px 0px 0px 4px; float: left; }
			.pitemtotalnav{ font-style: normal; font-size: 13px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 126px; padding: 0px 0px 0px 0px; height: 23px; margin: 0px 0px 0px 14px; float: left; }
			.textototalbottomleft{ background: #EDEDEE; color:#555555; font-style: normal; font-size: 13px; font-weight: normal; text-align: right; line-height: 21px; display: block; width: 160px; padding: 13px 10px; margin: 0px; float: left; }
			.textototalbottomright{ background: #EDEDEE; color:#555555; font-style: normal; letter-spacing:0px; font-size: 13px; font-weight: normal; text-align: right; line-height: 21px; display: block; width: 126px; padding: 13px 10px; margin: 0px; float: right; }
			.totalbottomleft{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 22px; font-weight: bold; text-align: right; line-height: 26px; display: block; width: 160px; padding: 10px; margin: 0px; float: left; }
			.totalbottomright{ background: #EDEDEE; color:'.$appColorBrand.'; font-style: normal; letter-spacing:-1px; font-size: 22px; font-weight: bold; text-align: right; line-height: 26px; display: block; width: 126px; padding: 10px; margin: 0px; float: right; }
			.classhr{ color:#EDEDEE; height: 2px; }
	</style>
</head>
<body>
';
...
if ($factura_original !== '' || $motivo_rectificado !== '') {
    $html .= '<div style="width: 100%; margin-bottom: 8px;">';
    if ($factura_original !== '') {
        $html .= '<p class="texto-c">Factura original: ' . htmlspecialchars($factura_original, ENT_QUOTES, 'UTF-8') . '</p>';
    }
    if ($motivo_rectificado !== '') {
        $html .= '<p class="texto-c">Motivo rectificado: ' . htmlspecialchars($motivo_rectificado, ENT_QUOTES, 'UTF-8') . '</p>';
    }
    $html .= '</div>';
}
...
FROM facturas_rectificativas_rel_articulos_simplificadas fra
...
if ($lineas_impresas === 0 && abs($total_factura) > 0.0) {
    ...
    $suma_base_imponible += $base_linea;
    $suma_iva_21 += $iva_linea;
    $descripcion_articulo = $motivo_rectificado !== '' ? $motivo_rectificado : 'Abono factura rectificativa';
    $html .= '
			  <div class="itemsinvoice">
...
$html_ticketbai = fiskalyHtmlBloqueQrImpresion($id_qr, $qr_img_html, $factura_regimen_qr);
...
$mpdfone->OutputHttpDownload('factura_rectificativa_simplificada_' . $id_factura . '.pdf');
exit;
