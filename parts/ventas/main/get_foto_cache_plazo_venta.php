<?php
/**
 * Devuelve nombre_foto del cache (cobro plazo venta) por id_foto + id_sucursal + id_venta + id_plazo_venta.
 */
...
try {
    ...
    if ($id_foto <= 0 || $id_sucursal <= 0 || $id_venta <= 0 || $id_plazo <= 0) {
    ...
    mysqli_stmt_bind_param($stmt, 'iiii', $id_foto, $id_sucursal, $id_venta, $id_plazo);
    ...
    echo json_encode(['success' => true, 'nombre_foto' => $nombre, 'tiene_foto' => $nombre !== ''], JSON_UNESCAPED_UNICODE);
} catch (Exception $e) {
