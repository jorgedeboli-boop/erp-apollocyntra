...
/**
 * Genera el registro base de informe_diario por sucursal habilitada (si no existe en la fecha).
 * Copia adaptada de CRON/generar-informe.php (no modifica CRON/).
 * year_rel se toma de la fecha manual, no de hoy.
 */
...
$yearRel = (int) date('Y', strtotime($fecha_informe_today));

cron_linea('>> Paso: generar-informe | fecha=' . $fecha_informe_today . ' | semana=' . $numeroSemana . ' | year_rel=' . $yearRel);
...
while ($sucursal = mysqli_fetch_assoc($resultadoSucursales)) {
    ...
    if ($idInformeReady <= 0) {
        ...
        cron_linea('    El informe de esa fecha ya existe Nº ' . $idInformeReady);
    }
}
