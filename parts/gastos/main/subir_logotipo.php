...
try {
    ...
    $nombre_archivo = 'logotipo_gasto_' . $id_gasto . '_' . time() . '.' . $extension;
    $ruta_destino = '../../../photos/' . $nombre_archivo;
    
    // Si ya existe un logotipo, eliminarlo primero
    $query_actual = "SELECT logotipo_gasto FROM gastos WHERE id_gasto = ?";
    $stmt_actual = mysqli_prepare($conexion, $query_actual);
    mysqli_stmt_bind_param($stmt_actual, 'i', $id_gasto);
    ...
    if ($result_actual && mysqli_num_rows($result_actual) > 0) {
        ...
        $logotipo_anterior = $row_actual['logotipo_gasto'];
    ...
    if (move_uploaded_file($archivo['tmp_name'], $ruta_destino)) {
        ...
        $query_update = "UPDATE gastos SET logotipo_gasto = ? WHERE id_gasto = ?";
        $stmt_update = mysqli_prepare($conexion, $query_update);
        mysqli_stmt_bind_param($stmt_update, 'si', $nombre_archivo, $id_gasto);
        
        if (mysqli_stmt_execute($stmt_update)) {
            // Log de la acción - Comentado temporalmente para debug
            // log_accion('Logotipo de gasto actualizado', 'gastos', $id_gasto);
            
            // Debug: verificar que se actualizó correctamente
            error_log("Logotipo actualizado en BD: gasto_id={$id_gasto}, archivo={$nombre_archivo}");
