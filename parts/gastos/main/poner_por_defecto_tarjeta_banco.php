...
try {
    ...
    // Verificar que la tarjeta pertenece a la gasto
    $query_verificar = "SELECT gasto_tarjeta_id FROM tarjetas_banco_gastos WHERE id_tarjeta_banco = ?";
    ...
    if ($row_verificar['gasto_tarjeta_id'] != $id_gasto) {
        mysqli_stmt_close($stmt_verificar);
        mysqli_close($conexion);
        echo json_encode(array('success' => false, 'message' => 'La tarjeta no pertenece a esta gasto'));
        exit;
    }
    
    mysqli_stmt_close($stmt_verificar);
    
    // Primero, quitar el flag de por defecto de todas las tarjetas de la gasto
    $query_reset = "UPDATE tarjetas_banco_gastos SET por_defecto = 'false' WHERE gasto_tarjeta_id = ?";
    ...
    $query_update = "UPDATE tarjetas_banco_gastos SET por_defecto = 'true' WHERE id_tarjeta_banco = ?";
