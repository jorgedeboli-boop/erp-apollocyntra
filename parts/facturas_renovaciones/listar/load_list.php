<?php
/**
 * Server-side processing para DataTable de facturas simplificadas.
 */
...
try {
    ...
    $queryBase = '
        FROM (
            SELECT id_factura, numero_factura, prefijo_factura, fecha_factura, hora_factura,
                   ...
                   factura_regimen, id_rel_factura_fiskaly, id_sucursal, rel_id_empresa,
                   \'historico\' AS origen_lista
    ...
    $query_total = 'SELECT (
        (SELECT COUNT(*) FROM facturas_simplificadas WHERE tipo_factura = \'renovaciones\')
        + (SELECT COUNT(*) FROM facturas WHERE tipo_factura = \'renovaciones\')
    ) AS total';
    ...
    $orderBy = isset($columnMap[$orderColumn]) && in_array($columnMap[$orderColumn], $allowedColumns, true) ? $columnMap[$orderColumn] : 'f.fecha_factura';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $origenLista = ((string) ($row['origen_lista'] ?? '') === 'facturas') ? 'facturas' : 'historico';
