...
function formatear_euro_informe_global($valor)
{
...
try {
    ...
    $stmt = mysqli_prepare(
        $conexion,
        'UPDATE informe_diario SET total_gastos = ? WHERE id_informe = ? LIMIT 1'
    );
    ...
    mysqli_stmt_bind_param($stmt, 'di', $total_gastos, $id_informe);
    ...
    echo json_encode([
        ...
        'yulinfo' => 0,
        'total_gastos_formatted' => formatear_euro_informe_global($total_gastos),
        'yulinfo_formatted' => formatear_euro_informe_global(0),
    ]);
} catch (Exception $e) {
