...
document.addEventListener('DOMContentLoaded', function () {
  const tableEl = document.querySelector('.datatables-cuentas-banco-config');
  let dt;
  ...
  function crearFiltros() {
    const contBanco = document.querySelector('.filtro_banco_cuenta');
    const contEmpresa = document.querySelector('.filtro_empresa_cuenta');
    if (!contBanco || !contEmpresa) return;

    contBanco.innerHTML = '<select id="filtro_banco_cuenta" class="form-select select2-custom select2"><option value="">Banco</option></select>';
    contEmpresa.innerHTML = '<select id="filtro_empresa_cuenta" class="form-select select2-custom select2"><option value="">Empresa</option></select>';

    fetch('parts/cuentas_banco_config/listar/get_filtros.php')
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (!data.success) return;
        const selBanco = document.getElementById('filtro_banco_cuenta');
        const selEmpresa = document.getElementById('filtro_empresa_cuenta');
  ...
  dt = new DataTable(tableEl, {
    ...
    createdRow: function (row, data) {
      ...
      $(row).on('click', function (e) {
        if (!$(e.target).closest('a, button').length) {
          window.location.href = 'cuenta_banco_config.php?id=' + data[0];
        }
      });
    },
    columnDefs: [
      {
        ...
        render: function (data) {
          ...
          return id
            ? '<a href="cuenta_banco_config.php?id=' + id + '" class="fw-semibold text-primary">' + id + '</a>'
            : '—';
        }
      },
      {
      ...
      topEnd: {
        features: [
          { search: { placeholder: 'Buscar...', text: '_INPUT_' } },
          {
            buttons: [
              {
                text: '<i class="icon-base ri ri-add-line icon-sm me-0 me-sm-2 d-sm-none d-inline-block"></i><span class="d-none d-sm-inline-block">Nueva cuenta</span>',
                className: 'add-new btn btn-primary',
                action: function () {
                  window.location.href = 'crear_cuenta_banco_config.php';
                }
              }
            ]
          }
    ...
    ajax: {
      url: 'parts/cuentas_banco_config/listar/load_list.php',
      type: 'POST',
      data: function (d) {
        const fb = document.getElementById('filtro_banco_cuenta');
        const fe = document.getElementById('filtro_empresa_cuenta');
  ...
  window.dt_cuentas_banco_config = dt;
