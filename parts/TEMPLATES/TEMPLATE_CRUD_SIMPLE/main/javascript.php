...
<?php
$vTablesDatatablesLoad = filemtime(__DIR__ . '/../../../proveedores/main/tables-datatables-load.js');
$vTablesDatatablesLoadCuentasBancarias = filemtime(__DIR__ . '/../../../proveedores/main/tables-datatables-load-cuentas-bancarias.js');
$vTablesDatatablesLoadTarjetasBanco = filemtime(__DIR__ . '/../../../proveedores/main/tables-datatables-load-tarjetas-banco.js');
?>
...
document.addEventListener('DOMContentLoaded', function() {
    // Cargar logotipo del proveedor al inicializar
    cargarLogotipoProveedor();
});

/**
 * Función para cargar el logotipo del proveedor
 */
function cargarLogotipoProveedor() {
    if (!idProveedor) {
        console.error('ID de proveedor no disponible');
        mostrarSinLogotipo();
        return;
    }
    ...
    // console.log('Haciendo fetch a:', `parts/proveedores/main/get_logotipo_info.php?id=${idProveedor}`);

    fetch(`parts/proveedores/main/get_logotipo_info.php?id=${idProveedor}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if (data.success && data.logotipo) {
                ...
                mostrarLogotipo(data.logotipo, idProveedor);
            } else {
 ...
 */
function mostrarLogotipo(nombreArchivo, idProveedor) {
    const contenedor = document.getElementById('contenedor_logotipo_actual');
    ...
    if (!botonEliminarExistente) {
        ...
        botonEliminar.onclick = () => eliminarLogotipo(idProveedor);
...
function subirLogotipo() {
    ...
    fetch('parts/proveedores/main/subir_logotipo.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
 ...
 */
function eliminarLogotipo(idProveedor) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // console.log('Enviando petición a eliminar_logotipo.php con id_proveedor:', idProveedor);

            fetch('parts/proveedores/main/eliminar_logotipo.php', {
                method: 'POST',
                headers: {
