/**
 * Page Proveedores List - Fixed Translations
 */
...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_proveedores_table = document.querySelector('.datatables-proveedores');

  // Variable global para DataTable
  let dt_proveedores;

  // Proveedores datatable
  if (dt_proveedores_table) {
    dt_proveedores = new DataTable(dt_proveedores_table, {
      ...
      ajax: 'parts/proveedores/listar/load_list.php',
      
      // ⚠️ IMPORTANTE: Si necesitas colores dinámicos, tráelos desde la BD en load_list.php
      language: DATATABLES_SPANISH,
      columns: [
        // columns according to JSON
        { data: 0 }, // ID del proveedor
        { data: 1 }, // Nombre del proveedor
        { data: 2 }, // Dirección
        { data: 3 }, // Población
        { data: 4 }, // Provincia
        { data: 5 }, // Teléfono
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            const proveedorId = full[0]; // ID del proveedor
            
            return `
              <div class="d-flex align-items-center">
                <a href="proveedor.php?id=${proveedorId}" class="btn btn-icon btn-text-secondary rounded-pill me-1" title="Ver proveedor">
                ...
                <div class="dropdown d-inline-block">
                  ...
                  <div class="dropdown-menu dropdown-menu-end m-0">
                    <a href="editar_proveedor.php?id=${proveedorId}" class="dropdown-item">
                      <i class="icon-base ri ri-pencil-line me-2"></i>
                      <span class="align-middle">Editar</span>
                    </a>
                    <a href="javascript:;" class="dropdown-item" onclick="eliminarProveedor(${proveedorId})">
        ...
        topEnd: {
          features: [
            ...
            },
            {
              buttons: [
                {
                  text: '<i class="icon-base ri ri-add-line icon-sm me-0 me-sm-2 d-sm-none d-inline-block"></i><span class="d-none d-sm-inline-block">Nuevo Proveedor</span>',
      ...
      // ⚠️ PATRÓN ESTÁNDAR: initComplete para crear filtros con Select2
      // Si tu módulo tiene filtros en el card-header, descomenta y adapta este código:
      /*
      initComplete: function () {
        const api = this.api();

        // Helper function to create a select dropdown with Select2
        ...
        // Ejemplo: createFilter(3, '.user_sucursal', 'FilterSucursal', 'Seleccionar Sucursal');
      },
      */
      ...
      responsive: {
        details: {
          display: DataTable.Responsive.display.modal({
            header: function (row) {
              const data = row.data();
              return 'Detalles de ' + data[1]; // Nombre del proveedor
            }
          }),
          type: 'column',
    ...
    window.dt_proveedores = dt_proveedores;
    ...
    function deleteRecord(event) {
      ...
      if (row) {
        dt_proveedores.row(row).remove().draw();
      }
    }

    function bindDeleteEvent() {
      const proveedoresListTable = document.querySelector('.datatables-proveedores');
      const modal = document.querySelector('.dtr-bs-modal');

      if (proveedoresListTable && proveedoresListTable.classList.contains('collapsed')) {
        if (modal) {
        ...
        const tableBody = proveedoresListTable?.querySelector('tbody');
        if (tableBody) {
...
/**
 * ⚠️ NOTAS IMPORTANTES PARA EL TEMPLATE:
 * 
 * 1. IDIOMA: Usar siempre `language: DATATABLES_ESPAÑOL`
 * 2. COLORES DINÁMICOS: Nunca hardcodear, traer desde BD en load_list.php
 * 3. AJAX: Configurar una sola vez (línea 29)
 * 4. LAYOUT: Usar el patrón estándar con bottomStart y bottomEnd
 * 5. RESPONSIVE: Incluir siempre modal para móviles
 * 6. TIMEOUT: Incluir setTimeout para ajustar clases CSS
 * 
 * REFERENCIA: parts/usuarios/listar/tables-datatables-load.js
 */
