...
function formatear_euro_reporte_actual_export($valor)
{
    return number_format((float) $valor, 2, ',', '.') . ' €';
}

function formatear_gramos_reporte_actual_export($valor)
{
    return number_format((float) $valor, 2, ',', '.') . ' gr';
}

function formatear_fecha_reporte_actual_export($fecha)
{
...
try {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $data[] = [
            formatear_fecha_reporte_actual_export($row['fecha_informe']),
            formatear_euro_reporte_actual_export($row['total_euros_lotes_compra_oro']),
            formatear_gramos_reporte_actual_export($row['total_gramos_compra_oro']),
            formatear_euro_reporte_actual_export($row['total_euros_lotes_compra_plata']),
            formatear_gramos_reporte_actual_export($row['total_gramos_compra_plata']),
            formatear_euro_reporte_actual_export($row['total_euros_lotes_empenios']),
            formatear_gramos_reporte_actual_export($row['total_gramos_empenios_oro']),
            formatear_euro_reporte_actual_export($row['total_euros_renovaciones']),
            formatear_euro_reporte_actual_export($row['total_euros_ventas']),
            formatear_euro_reporte_actual_export($row['total_gastos']),
            formatear_euro_reporte_actual_export($row['stock_valorizado_eruo']),
