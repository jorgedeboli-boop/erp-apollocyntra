...
function formatear_euro_reporte_actual($valor)
{
    return number_format((float) $valor, 2, ',', '.') . ' €';
}

function formatear_gramos_reporte_actual($valor)
{
    return number_format((float) $valor, 2, ',', '.') . ' gr';
}

function formatear_fecha_reporte_actual($fecha)
{
...
try {
    ...
    $result_total = mysqli_query($conexion, 'SELECT COUNT(*) AS total FROM informe_actual');
    ...
    while ($row = mysqli_fetch_assoc($result_data)) {
        ...
        $data[] = [
            formatear_fecha_reporte_actual($row['fecha_informe']),
            formatear_euro_reporte_actual($row['total_euros_lotes_compra_oro']),
            formatear_gramos_reporte_actual($row['total_gramos_compra_oro']),
            formatear_euro_reporte_actual($row['total_euros_lotes_compra_plata']),
            formatear_gramos_reporte_actual($row['total_gramos_compra_plata']),
            formatear_euro_reporte_actual($row['total_euros_lotes_empenios']),
            formatear_gramos_reporte_actual($row['total_gramos_empenios_oro']),
            formatear_euro_reporte_actual($row['total_euros_renovaciones']),
            formatear_euro_reporte_actual($row['total_euros_ventas']),
            formatear_euro_reporte_actual($total_gastos),
            formatear_euro_reporte_actual($row['stock_valorizado_eruo']),
