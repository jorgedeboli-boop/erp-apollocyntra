...
/**
 * Informe diario: operaciones por transferencia (entradas y salidas).
 */

$ctx = cron_informe_contexto('informes-operaciones-transferencias');
if (!$ctx) {
    return;
}
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-operaciones-transferencias preparando consulta de informes.');
    return;
}
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    if ($totalEntrada != 0) {
        $sqlUpdate = "UPDATE informe_diario SET total_operaciones_trasnferencia_entrada = ? WHERE id_informe = ?";
    ...
    if ($totalSalida != 0) {
        $sqlUpdate = "UPDATE informe_diario SET total_operaciones_trasnferencia_salida = ? WHERE id_informe = ?";
        $stmtUpdate = mysqli_prepare($conexion, $sqlUpdate);
        if ($stmtUpdate) {
