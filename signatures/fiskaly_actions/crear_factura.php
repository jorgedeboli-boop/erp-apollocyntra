...
try {
    ...
    if($formato_factura == 'articulos'){
        require_once 'json_invoice_COMPLETE.php';
    }elseif($formato_factura == 'oro_inversion'){
        require_once 'json_invoice_COMPLETE_oro_inversion.php';
    }

    $uuid_invoice = generarUUIDv4();

     // Actualizar client_id_fiskaly con $idClient
     $query_update = "UPDATE facturas_fiskaly_cache SET client_id_fiskaly = ?, invoice_id_fiskaly = ? WHERE id_factura = ?";
     ...
     $stmt_update->bind_param('ssi', $idClient, $uuid_invoice, $factura_id_fiskaly);
     if (!$stmt_update->execute()) {
         $stmt_update->close();
         throw new Exception('Error al actualizar client_id_fiskaly: ' . $stmt_update->error);
     }
     $stmt_update->close();
    
    // Verificar que $json_string esté definida
    if (!isset($json_string)) {
        throw new Exception('Error: $json_string no está definida después de incluir json_invoice_COMPLETE.php o json_invoice_COMPLETE_oro_inversion.php');
    }
