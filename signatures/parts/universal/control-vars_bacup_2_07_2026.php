...
} elseif ((int) $usuario_privilegio_id === 8) {
    $dashboard_redirect_url = 'dashboard_auditorias.php';
} elseif ((int) $usuario_privilegio_id === 17) {
...
if (mysqli_num_rows($resultitemsSections) == 0) {
    ...
    if($sucursal_section_item == "true"){
        if($_SESSION['sucursal_section'] == "false"){
            $id_action_user = "37";
            $texto_action_user = "$usuario intenta acceder a la seccion ".$itemname." que es tipo sucursal";
            registrar_accion_usuario($usuario_id, $id_action_user, $texto_action_user, $usuario_sucursal, $id_type_Item, $url_completa);
            header('Location: ' . $dashboard_redirect_url . '?error=no tienes acceso a esta seccion 1');
            exit();
        }
    }else{
        if($_SESSION['sucursal_section'] == "true"){
            $id_action_user = "38";
            $texto_action_user = "$usuario intenta acceder a la seccion ".$itemname." de central";
            registrar_accion_usuario($usuario_id, $id_action_user, $texto_action_user, $usuario_sucursal, $id_type_Item, $url_completa);
            header('Location: ' . $dashboard_redirect_url . '?error=no tienes acceso a esta seccion 2');
            exit();
        }
    }
