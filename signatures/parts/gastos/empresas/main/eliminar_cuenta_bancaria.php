...
try {
    ...
    $query = "SELECT id_cuenta_banco, empresa_cuenta_id, por_defecto FROM cuentas_banco_empresas WHERE id_cuenta_banco = ?";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $query_delete = "DELETE FROM cuentas_banco_empresas WHERE id_cuenta_banco = ?";
        $stmt_delete = mysqli_prepare($conexion, $query_delete);
        mysqli_stmt_bind_param($stmt_delete, 'i', $id_cuenta_banco);
        
        if (mysqli_stmt_execute($stmt_delete)) {
            // Log de la acción
            // log_accion('Cuenta bancaria eliminada', 'empresas', $row['empresa_cuenta_id']);
