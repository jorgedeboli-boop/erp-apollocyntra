...
try {
    ...
    $query_insert = "INSERT INTO tarjetas_banco_gastos 
                     (numerotarjeta, banco_tarjeta, gasto_tarjeta_id, fecha_creacion, creado_por, por_defecto) 
                     VALUES (?, ?, ?, CURDATE(), ?, 'false')";
    
    $stmt_insert = mysqli_prepare($conexion, $query_insert);
    mysqli_stmt_bind_param($stmt_insert, 'ssii', $numerotarjeta, $banco_tarjeta, $id_gasto, $usuario_actual);
    
    if (mysqli_stmt_execute($stmt_insert)) {
        $id_tarjeta_nueva = mysqli_insert_id($conexion);
        
        // Log de la acción
        // log_accion('Tarjeta banco creada', 'gastos', $id_gasto);
