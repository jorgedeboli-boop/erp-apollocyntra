...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_sucursales_gasto_table = document.querySelector('.datatables-sucursales-gasto'),
  ...
  let dt_sucursales_gasto;

  // Sucursales de gasto datatable
  if (dt_sucursales_gasto_table) {
    dt_sucursales_gasto = new DataTable(dt_sucursales_gasto_table, {
      ...
      ajax: {
        url: 'parts/gastos/main/get_sucursales_gasto.php',
        type: 'POST',
        data: function (d) {
          // Agregar el ID de la gasto a la petición
          d.id_gasto = window.idGasto || 0;
          return d;
        },
        error: function (xhr, error, thrown) {
          ...
          if (dt_sucursales_gasto_table) {
            dt_sucursales_gasto_table.innerHTML = `
      ...
      language: {
        ...
        emptyTable: "No hay sucursales disponibles para esta gasto",
...
function recargarSucursalesEmpresa() {
  if (window.dt_sucursales_gasto) {
    window.dt_sucursales_gasto.ajax.reload(null, false);
  }
}
