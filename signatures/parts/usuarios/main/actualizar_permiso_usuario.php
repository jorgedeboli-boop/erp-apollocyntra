...
if ($id_usuario <= 0 || $id_jerarquia <= 0 || $item_id <= 0 || ($estado !== 'activo' && $estado !== 'no_activo')) {
...
try {
    ...
    $stmtCheckJerarquia = mysqli_prepare(
        $conexion,
        'SELECT COUNT(*) AS total FROM relItemsLevel WHERE relIdItems = ? AND relIdUsersLevel = ?'
    );
    if (!$stmtCheckJerarquia) {
        throw new Exception('Error al preparar consulta de jerarquía: ' . mysqli_error($conexion));
    }

    mysqli_stmt_bind_param($stmtCheckJerarquia, 'ii', $item_id, $id_jerarquia);
    ...
    if ((int) $totalJerarquia > 0) {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Este permiso pertenece a la jerarquía y no se puede modificar aquí',
        ]);
        exit();
    }
    ...
    if ($estado === 'activo') {
        $stmtCheck = mysqli_prepare(
            $conexion,
            'SELECT COUNT(*) AS total FROM relItemsUser WHERE relIdItems = ? AND relIdUser = ?'
        );
        if (!$stmtCheck) {
            throw new Exception('Error al preparar consulta: ' . mysqli_error($conexion));
        }

        mysqli_stmt_bind_param($stmtCheck, 'ii', $item_id, $id_usuario);
        ...
        if ((int) $total === 0) {
            $stmtInsert = mysqli_prepare(
                $conexion,
                'INSERT INTO relItemsUser (relIdItems, relIdUser, relIdUsersLevel) VALUES (?, ?, ?)'
            );
            if (!$stmtInsert) {
                throw new Exception('Error al preparar inserción: ' . mysqli_error($conexion));
            }

            mysqli_stmt_bind_param($stmtInsert, 'iii', $item_id, $id_usuario, $id_jerarquia);
        ...
        $stmtDelete = mysqli_prepare(
            $conexion,
            'DELETE FROM relItemsUser WHERE relIdItems = ? AND relIdUser = ?'
        );
        if (!$stmtDelete) {
            throw new Exception('Error al preparar eliminación: ' . mysqli_error($conexion));
        }

        mysqli_stmt_bind_param($stmtDelete, 'ii', $item_id, $id_usuario);
    ...
    echo json_encode([
        'success' => true,
        'message' => $estado === 'activo' ? 'Acceso personalizado activado' : 'Acceso personalizado desactivado',
    ]);
} catch (Exception $e) {
    error_log('Error en usuarios/main/actualizar_permiso_usuario: ' . $e->getMessage());
