...
function formatear_euro_informe($valor)
{
...
try {
    ...
    $stmt = mysqli_prepare(
        $conexion,
        "UPDATE informe_semanal SET total_gastos = ?, yulinfo = ? WHERE id_informe = ? LIMIT 1"
    );
    ...
    echo json_encode([
        ...
        'yulinfo' => $yulinfo,
        'total_gastos_formatted' => formatear_euro_informe($total_gastos),
        'yulinfo_formatted' => formatear_euro_informe($yulinfo)
    ]);
} catch (Exception $e) {
