...
foreach ($articulos as $row) {
    $total_venta += isset($row['precio']) ? (float) $row['precio'] : 0;
}

$importe_esperado = $total_venta;
if ($tipo_venta === 'plazos' && $numero_plazos > 0) {
    $importe_esperado = $total_venta / $numero_plazos;
}
...
try {
    ...
    foreach ($articulos as $art) {
        ...
        try {
            ...
            error_log('trazabilidad_articulos_venta: ' . $e->getMessage());
        }

    }

    if ($venta_plazos_db === 'si') {
        $concepto_caja = "Cobro de plazo de la venta Nº ". $id_venta_sucursal." (Cuota Nº1)";
        $grupos_caja = "Venta a plazos";
    }else{
        $concepto_caja = "Venta Nº " . $id_venta_sucursal;
        $grupos_caja = "Venta de articulo";
    }
    ...
    if ($venta_plazos_db === 'si') {
        ...
        $stmtVpPagado = mysqli_prepare(
            $conexion,
            'INSERT INTO ventas_plazos (id_venta, estado, fecha_cobrado, importe, metodo_pago) VALUES (?, ?, ?, ?, ?)'
        );
        if (!$stmtVpPagado) {
    ...
    /**
     * Factura completa obligatoria (importe con IVA incluido): venta no a plazos y total > 400 €.
     * Si falla la generación, la venta sigue válida (ya confirmada); solo se registra en log.
     */
    $id_factura_generada = 0;
    if ($venta_plazos_db === 'no' && $total_venta > 400.0) {
        try {
            ...
            $prefijo_f = $rowP ? substr((string) ($rowP['pref'] ?? ''), 0, 5) : '';

            $numero_sig = (int) obtenerNumeroFactura($sucursal_venta);
            ...
            if ($stmtLines) {
                mysqli_stmt_bind_param($stmtLines, 'ii', $id_venta_pk, $sucursal_venta);
                ...
                while ($rowL = mysqli_fetch_assoc($resLines)) {
                    ...
                    $filasItems[] = [
                        'id_rel_sucursal' => $sucursal_venta,
            ...
            error_log('insertar_venta: factura automática no generada: ' . $exFact->getMessage());
        }
    }
