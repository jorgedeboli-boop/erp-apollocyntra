...
try {
    ...
    $query = "SELECT id_tarjeta_banco, gasto_tarjeta_id, por_defecto FROM tarjetas_banco_gastos WHERE id_tarjeta_banco = ?";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $query_delete = "DELETE FROM tarjetas_banco_gastos WHERE id_tarjeta_banco = ?";
        $stmt_delete = mysqli_prepare($conexion, $query_delete);
        mysqli_stmt_bind_param($stmt_delete, 'i', $id_tarjeta_banco);
        
        if (mysqli_stmt_execute($stmt_delete)) {
            // Log de la acción
            // log_accion('Tarjeta banco eliminada', 'gastos', $row['gasto_tarjeta_id']);
