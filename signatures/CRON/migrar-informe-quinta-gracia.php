...
/**
 * Informe diario: copia informes abiertos de hoy a Manager Quinta Gracia (manager_main_222).
 */

$ctx = cron_informe_contexto('migrar-informe-quinta-gracia');
if (!$ctx) {
    return;
}
...
if (!$conexionManager) {
    cron_linea('ERROR migrar-informe-quinta-gracia: no se pudo conectar a Manager Quinta Gracia.');
    return;
}

$sqlInformes = "SELECT * FROM informe_diario
                WHERE fecha_informe = ?
                  AND estado_informe = 'abierto'
                ORDER BY id_informe ASC";
$stmtInformes = mysqli_prepare($conexion, $sqlInformes);
if (!$stmtInformes) {
    cron_linea('ERROR migrar-informe-quinta-gracia preparando SELECT.');
...
if (!mysqli_stmt_execute($stmtInformes)) {
    cron_linea('ERROR migrar-informe-quinta-gracia ejecutando SELECT.');
    mysqli_stmt_close($stmtInformes);
    mysqli_close($conexionManager);
    return;
}

$sqlExiste = 'SELECT id_informe FROM informe_diario WHERE id_informe = ? LIMIT 1';
...
while ($fila = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    if ($stmtExiste) {
        ...
        if ($yaExiste) {
            $totalOmitidos++;
            cron_linea('  - Informe ' . $idInforme . ' ya existe en Manager, se omite.');
            continue;
        }
    }
    ...
    $sqlInsert = 'INSERT INTO informe_diario (' . $columnasSql . ') VALUES (' . $valoresSql . ')';
...
if ($totalMigrados === 0 && $totalOmitidos === 0 && $totalErrores === 0) {
    cron_linea('  - No hay informes abiertos para migrar.');
} else {
    cron_linea(
        '  - Migrados: ' . $totalMigrados
        . ' | omitidos (ya existian): ' . $totalOmitidos
