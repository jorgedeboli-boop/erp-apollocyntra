...
/**
 * Informe actual: totales de compras de oro por sucursal (lotes en estado compra).
 * Lee lotes_joyeria (vista unificada alimentada por triggers desde lotes_{id}).
 */

$conexion = cron_obtener_conexion();
if (!$conexion) {
    cron_linea('ERROR informes-actual-compras-oro: sin conexion a base de datos.');
    return;
}
...
cron_linea('>> Paso: informes-actual-compras-oro | fecha=' . $fecha_informe_actual_today);
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-actual-compras-oro preparando consulta de informes: ' . mysqli_error($conexion));
    return;
}

mysqli_stmt_bind_param($stmtInformes, 's', $fecha_informe_actual_today);

if (!mysqli_stmt_execute($stmtInformes)) {
    cron_linea('ERROR informes-actual-compras-oro consultando informes: ' . mysqli_stmt_error($stmtInformes));
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    $fechaInforme = isset($informe['fecha_informe']) ? (string) $informe['fecha_informe'] : $fecha_informe_actual_today;
