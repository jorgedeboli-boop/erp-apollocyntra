...
/**
 * Informe actual: totales globales de empeños (oro + plata).
 * Lee lotes_joyeria (vista unificada alimentada por triggers desde lotes_{id}).
 */

$ctx = cron_informe_actual_contexto('informes-actual-empenyos-global');
if (!$ctx) {
    return;
}
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-actual-empenyos-global preparando consulta de informes.');
    return;
}
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    if ($totalLotes > 0) {
        ...
        cron_linea('  - Informe ' . $idInforme . ' | empenyos global | lotes=' . $totalLotes);
    }
