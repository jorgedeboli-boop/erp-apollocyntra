...
/**
 * Paso 4: desactivar en PrestaShop articulos vendidos o retirados hoy en tienda (articulo_web).
 */

$conexion = cron_obtener_conexion();
$conexionWeb = cron_obtener_conexion_web();
if (!$conexion || !$conexionWeb) {
    cron_linea('ERROR verificar_productos_web_4: sin conexion a base de datos.');
    return;
}

$idUsuarioCron = 1;

cron_linea('>> Paso: verificar_productos_web_4');
...
if (!$stmtProductoWeb || !$stmtUpdateProducto || !$stmtUpdateProductoShop) {
    cron_linea('ERROR verificar_productos_web_4 preparando consultas web.');
    return;
}
 ...
 * @param string $descripcionCron
 * @param string $accionTrazabilidad
 * @param int $idUsuarioCron
 * @return int
 */
function cron_web4_procesar_articulos(
    $conexion,
    $conexionWeb,
    $stmtProductoWeb,
    $stmtUpdateProducto,
    ...
    if (!$resultado) {
        cron_linea('ERROR verificar_productos_web_4 consultando articulos: ' . mysqli_error($conexion));
        return 0;
    }
...
$totalVendidos = cron_web4_procesar_articulos(
    $conexion,
    $conexionWeb,
    $stmtProductoWeb,
    $stmtUpdateProducto,
    ...
    'articulo vendido en tienda y atualizado en al web como fuera de stock SKU: %sku% id presatshop Nº %id_prestashop%',
    'El articulo vendido en tienda y atualizado en al web como fuera de stock SKU: %sku% id presatshop Nº %id_prestashop%',
    'actualiza_web_vendido_tienda',
    $idUsuarioCron
);

$totalRetirados = cron_web4_procesar_articulos(
    $conexion,
    $conexionWeb,
    $stmtProductoWeb,
    $stmtUpdateProducto,
...
cron_linea('  - Actualizados vendidos en tienda: ' . $totalVendidos);
