...
/**
 * Informe diario: precio del oro vigente.
 */

$ctx = cron_informe_contexto('informes-precio-oro');
if (!$ctx) {
    return;
}
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-precio-oro preparando consulta de informes.');
    return;
}
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    $sqlUpdate = 'UPDATE informe_diario SET precio_oro = ? WHERE id_informe = ?';
