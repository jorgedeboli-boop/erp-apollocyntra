<?php
/**
 * PDF factura normal (tabla facturas). Facturas simplificadas: ?tipo=simplificada.
 *
...
if (isset($_GET['tipo']) && $_GET['tipo'] === 'simplificada') {
    $id_redir = isset($_GET['id_factura']) ? (int) $_GET['id_factura'] : 0;
    if ($id_redir > 0) {
        header('Location: factura_simplificada.php?id_factura=' . $id_redir);
    } else {
        header('Location: ../../dashboard.php');
    }
    exit;
        ...
        foreach ($fraRows as $row) {
            ...
            $html .= '
			  <div class="itemsinvoice">
					...
					<p class="pitemtotalnav" style="float: right;">' . parsearPrecioReturn($precio_rel_linea) . ' €</p>
				</div>
			';
        }
    }
...
FROM facturas_rel_articulos fra
