<?php
/**
 * PDF factura rectificativa (tabla facturas_rectificativas).
 *
...
if (!$factura_mpdf_buffer) {
    fiskalyRedirigirImpresionDesdeGet($id_factura, 'facturas_rectificativas', false);
}
...
if ($factura_mpdf_buffer && $id_factura > 0) {
    ...
    if ($conexionFiskaly) {
        $stmtFsk = mysqli_prepare(
            $conexionFiskaly,
            'SELECT factura_regimen, id_rel_factura_fiskaly FROM facturas_rectificativas WHERE id_factura = ? LIMIT 1'
        );
        if ($stmtFsk) {
            ...
            if ($rowFsk && fiskalyFacturaVinculadaEnCache((int) ($rowFsk['id_rel_factura_fiskaly'] ?? 0))
                && fiskalyScriptImpresionFactura((string) ($rowFsk['factura_regimen'] ?? 'false'), false) === 'factura_Bizkaia_tbai.php') {
                mysqli_close($conexionFiskaly);
                $_GET['tipo'] = 'rectificativa';
...
$queryFR = "SELECT
    ...
    dc.firma_cliente
FROM facturas_rectificativas AS FCTR
...
<meta charset="UTF-8" />
	<style>
			...
			.titulodocumento{ color:#555555; font-size: 30px; text-align: left; display: block; width: 350px; float: right; border-bottom: 2px solid '.$appColorBrand.'; line-height: 44px; margin:0px; }
			.texto-a{ color: '.$appColorBrand.'; font-style: italic; font-size: 16px; font-weight: normal; text-align: right; line-height: 29px; margin-right:5px; }
			.texto-b{ color: #555555; font-style: normal; font-size: 16px; font-weight: normal; line-height: 29px; }
			...
			.totaltopleft{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 27px; font-weight: bold; text-align: right; line-height: 58px; display: block; width: 174px; padding: 0px 13px 5px 0px; height: 54px; margin: 0px; float: left; }
			.totaltopright{ background: #EDEDEE; color: '.$appColorBrand.'; font-style: normal; letter-spacing:-1px; font-size: 22px; font-weight: bold; text-align: center; line-height: 58px; display: block; width: 143px; padding: 0px 3px 5px 0px; height: 54px; margin: 0px; float: right; }
			.itemnumbernav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 70px; padding: 0px; height: 28px; margin: 0px; float: left; }
			.itemdescripcionnav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 248px; padding: 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemunidadesnav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 80px; padding: 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itempreciounitarionav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 120px; padding: 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemivanav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 80px; padding: 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemtotalnav{ background: '.$appColorBrand.'; color: #ffffff; font-style: normal; font-size: 15px; font-weight: normal; text-align: center; line-height: 30px; display: block; width: 120px; padding: 0px; height: 28px; margin: 0px 0px 0px 6px; float: left; }
			.itemsinvoice{ width: 100%; height: 28px; margin-top: 3px; }
			.pitemnumbernav{ width: 70px; float:left; line-height: 26px; text-align: center; }
			.pitemdescripcionnav{ width: 248px; float:left; line-height: 26px; margin-left:6px; }
			.pitemunidadesnav{ width: 80px; float:left; line-height: 26px; text-align: center; margin-left:6px; }
			.pitempreciounitarionav{ width: 120px; float:left; line-height: 26px; text-align: right; margin-left:6px; }
			.pitemivanav{ width: 80px; float:left; line-height: 26px; text-align: center; margin-left:6px; }
			.pitemtotalnav{ width: 120px; float:left; line-height: 26px; text-align: right; margin-left:6px; }
		</style>
</head>
<body>
';
...
if(empty($id_cliente)){
    ...
    $html .= '
        ...
        <div style="width: 340px; height: 110px; float: right">
        
            <div style="width: 150px; height: 100px; float: left; padding: 5px 0px; ">
            
                <p class="texto-a" style="text-align: left;">Factura Nº: </p>
            
                <p class="texto-a" style="text-align: left;">Fecha emisión: </p>
                
                <p class="texto-a" style="text-align: left;">Forma de pago: </p>
    
                <p class="texto-a" style="text-align: left;">Rectifica factura Nº: </p>
            
            </div>
        
            <div style="width: 180px; height: 100px; float: left; padding: 5px 0px; ">
                ...
                <p class="texto-b" style="text-align: left;">'.htmlspecialchars($factura_original, ENT_QUOTES, 'UTF-8').'</p>
...
// Motivo rectificado encima del encabezado de la tabla
if ($motivo_rectificado !== '') {
    $html .= '<div style="width: 100%; margin-bottom: 8px;"><p class="texto-c">Motivo rectificado: ' . htmlspecialchars($motivo_rectificado, ENT_QUOTES, 'UTF-8') . '</p></div>';
}
...
FROM facturas_rectificativas_rel_articulos fra
...
$mpdfone->OutputHttpDownload('factura_rectificativa_' . $id_factura . '.pdf');
exit;
