...
/**
 * Genera o actualiza el registro base de informe_actual por sucursal habilitada (fecha de hoy).
 */

$conexion = cron_obtener_conexion();
if (!$conexion) {
    cron_linea('ERROR generar-informe_actual: sin conexion a base de datos.');
    return;
}
...
cron_linea('>> Paso: generar-informe_actual | fecha=' . $fecha_informe_actual_today . ' | semana=' . $numeroSemana);
...
if (!$resultadoSucursales) {
    cron_linea('ERROR generar-informe_actual consultando sucursales: ' . mysqli_error($conexion));
    return;
}
...
$sqlUpdateInforme = "UPDATE informe_actual
                     SET fecha_generado = NOW(),
                         hora_generado = NOW(),
                         semana_numero = ?,
                         year_rel = ?
                     WHERE id_informe = ?";
$stmtUpdateInforme = mysqli_prepare($conexion, $sqlUpdateInforme);

$sqlInsertInforme = "INSERT INTO informe_actual (
    sucursal_informe,
    fecha_informe,
    fecha_generado,
    hora_generado,
...
if (!$stmtExisteInforme || !$stmtUpdateInforme || !$stmtInsertInforme || !$stmtTestCron) {
    cron_linea('ERROR generar-informe_actual preparando consultas: ' . mysqli_error($conexion));
    if ($resultadoSucursales) {
    ...
    if ($stmtUpdateInforme) {
        mysqli_stmt_close($stmtUpdateInforme);
    }
...
while ($sucursal = mysqli_fetch_assoc($resultadoSucursales)) {
    ...
    mysqli_stmt_bind_param($stmtExisteInforme, 'is', $sucursalInforme, $fecha_informe_actual_today);
    ...
    if ($idInformeReady <= 0) {
        ...
        registrar_tareas_cron(
            'Genero informe actual Nº ' . $idInformeGenerate . ' de la Sucursal ' . $sucursalInforme
        );
        ...
        if (!mysqli_stmt_execute($stmtUpdateInforme)) {
            cron_linea('    ERROR actualizando informe existente: ' . mysqli_stmt_error($stmtUpdateInforme));
            continue;
        }

        cron_linea('    Informe existente actualizado Nº ' . $idInformeReady);
    }
}
