...
/**
 * Informe actual: empeños vencidos en la fecha del informe.
 * Lee lotes_joyeria (vista unificada alimentada por triggers desde lotes_{id}).
 */

$ctx = cron_informe_actual_contexto('informes-actual-empenyos-vencidos');
if (!$ctx) {
    return;
}
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-actual-empenyos-vencidos preparando consulta de informes.');
    return;
}
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    if ($totalLotes > 0) {
        ...
        $sqlUpdate = "UPDATE informe_actual
                      SET total_empenyos_vencidos = ?,
                          total_euros_empenyos_vencidos = ?,
                          total_gramos_empenios_vencidos = ?
                      WHERE id_informe = ?";
        $stmtUpdate = mysqli_prepare($conexion, $sqlUpdate);
        if ($stmtUpdate) {
            mysqli_stmt_bind_param($stmtUpdate, 'iddi', $totalLotes, $totalPrecio, $totalGramos, $idInforme);
            mysqli_stmt_execute($stmtUpdate);
            mysqli_stmt_close($stmtUpdate);
        }

        cron_linea('  - Informe ' . $idInforme . ' | vencidos | lotes=' . $totalLotes);
    }
