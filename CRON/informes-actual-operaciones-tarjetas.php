...
/**
 * Informe actual: operaciones con tarjeta.
 */

$ctx = cron_informe_actual_contexto('informes-actual-operaciones-tarjetas');
if (!$ctx) {
    return;
}
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-actual-operaciones-tarjetas preparando consulta de informes.');
    return;
}
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    if ($totalTarjeta != 0) {
        $sqlUpdate = "UPDATE informe_actual SET total_operaciones_tarjeta = ? WHERE id_informe = ?";
