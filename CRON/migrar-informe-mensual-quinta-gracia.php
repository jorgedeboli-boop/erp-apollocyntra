...
/**
 * Informe mensual: sincroniza informes abiertos a Manager Quinta Gracia (UPDATE por id_informe).
 */

$ctx = cron_informe_mensual_contexto('migrar-informe-mensual-quinta-gracia');
if (!$ctx) {
    return;
}
...
if (!$conexionManager) {
    cron_linea('ERROR migrar-informe-mensual-quinta-gracia: no se pudo conectar a Manager Quinta Gracia.');
    return;
}

$origenTestInicio = 'test cron mensual inicia migracion quinta';
...
$resultadoColumnas = mysqli_query($conexion, 'DESCRIBE informe_mensual');
...
if (empty($columnasPermitidas)) {
    cron_linea('ERROR migrar-informe-mensual-quinta-gracia: no se pudieron leer columnas.');
    mysqli_close($conexionManager);
    return;
}

$sqlInformes = "SELECT * FROM informe_mensual WHERE estado_informe = 'abierto' AND year_informe = ? ORDER BY id_informe ASC";
...
while ($fila = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    $sqlUpdate = 'UPDATE informe_mensual SET ' . implode(', ', $updates) . ' WHERE id_informe = ' . $idInforme;
    if (mysqli_query($conexionManager, $sqlUpdate)) {
        $totalMigrados++;
        cron_linea('  - Informe mensual ' . $idInforme . ' migrado a Manager.');
    } else {
        $totalErrores++;
        cron_linea('  ERROR UPDATE informe mensual ' . $idInforme . ': ' . mysqli_error($conexionManager));
    }
}

mysqli_stmt_close($stmtInformes);

$origenTestFin = 'test cron mensual finaliza migracion quinta';
