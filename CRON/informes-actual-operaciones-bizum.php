...
/**
 * Informe actual: operaciones Bizum.
 */

$ctx = cron_informe_actual_contexto('informes-actual-operaciones-bizum');
if (!$ctx) {
    return;
}
...
if (!$stmtInformes) {
    cron_linea('ERROR informes-actual-operaciones-bizum preparando consulta de informes.');
    return;
}
...
while ($informe = $resultadoInformes ? mysqli_fetch_assoc($resultadoInformes) : false) {
    ...
    if ($totalBizum != 0) {
        $sqlUpdate = "UPDATE informe_actual SET total_operaciones_bizum = ? WHERE id_informe = ?";
